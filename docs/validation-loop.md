...
| Stage | Tool | Catches |
|---|---|---|
| lint | ruff | style, imports, bugs (bugbear), security (`S`), complexity (`C90`), docstrings (`D`) |
...
| test | pytest | behavior, with branch **coverage** (floor enforced), seeded random order (**pytest-randomly**), and OpenAPI **schemathesis** fuzzing |

Each earns its place by catching a class of defect the others miss. The reasoning — and the
tools deliberately left out (standalone bandit, radon/xenon, vulture, mutation testing, SBOM,
diff-cover) — is recorded in
[ADR-0002](adr/0002-validation-loop-tool-selection.md).
...
The fix — bound the DTO field — is the pattern to copy: reject bad input at the edge with a 422,
...
plus the gitleaks and migration-safety jobs.
