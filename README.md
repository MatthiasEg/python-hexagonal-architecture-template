...
Production-grade hexagonal-architecture FastAPI template with layer boundaries enforced in CI, built for AI coding agents.
...
discipline, a fixed structure with a canonical example to mirror and one deterministic command
that defines "done", is what makes the codebase legible to AI coding agents.
...
[full-stack-fastapi-template](https://github.com/fastapi/full-stack-fastapi-template) instead.
Same goes if your service is thin CRUD where layered architecture is overhead you won't recoup.
...
nothing external. The integration tests and schemathesis use
[testcontainers](https://testcontainers.com/) to spin up Postgres, so they need Docker.
...
Hexagonal architecture (ports and adapters), three concentric layers. The one rule: imports
point inward only. Outer layers know about inner layers; the reverse never happens.
...
- **`domain/`** holds pure business logic: no framework imports, no I/O. Entities are immutable
  and own their identity and UTC timestamps. The `Task` entity encodes a status state machine
  (`OPEN -> IN_PROGRESS -> DONE`, plus `CANCELLED`); an illegal transition raises a domain
  error, not an HTTP status.
- **`application/`** is where use cases orchestrate domain objects through ports. `ports/input/`
  are driving contracts (what the app can do); `ports/output/` are driven contracts (repository,
  unit of work, logger) that infrastructure implements. It depends on `domain/` only.
- **`infrastructure/`** carries every concrete technology: FastAPI controllers and DTOs, the
...
When you need the domain to reach a database or an HTTP client, the dependency is backwards:
...
3. `application/ports/input/` and `use_cases/`: driving ports + use cases (unit-tested, no mocks)
...
One command is the definition of done, fail-fast and cheapest-first:
...
The loop is deterministic. Pinned versions, seeded test order (pytest-randomly), and derandomized
fuzzing (schemathesis) mean a re-run gives the same answer, so a flaky result is a real bug.
...
Not a slogan, but a set of files that let an agent add a feature and verify it unattended:

- **`AGENTS.md`** is the single source of truth (architecture, loop, constraints). `CLAUDE.md`
  imports it; `.github/copilot-instructions.md` and `.cursor/rules/` are thin pointers. There
  are no duplicated instructions to drift.
...
|---|---|---|---|---|
| Hexagonal / ports and adapters | Yes | No | N/A (guide) | Yes |
...
fills. A longer comparison, with an honest "when not to use", is in
